...
class Punto_de_venta(QMainWindow):
    def __init__(self):
        ...
        rango_usuario = rango_usuario[0][0] 
        print("Rango:", rango_usuario)
        if rango_usuario == "Administrador":
            self.texto_rango.setText(str("Cuenta con acceso"))
            ...
            self.celda_clave_usuario.setEnabled(True) 
            self.celda_contrasenia_usuario.setEnabled(True) 
            self.celda_nombre_usuario.setEnabled(True) 
            self.celda_correo_usuario.setEnabled(True) 
            self.celda_telefono_usuario.setEnabled(True)
            self.celda_nickname_usuario.setEnabled(True) 
            self.combo_box_tipo.setEnabled(True) 
            self.combo_box_estatus.setEnabled(True) 
            self.boton_registrar_usuario.setEnabled(True) 
    ...
    def fn_init_UI(self):
        ...
        importe_total = 0
        self.boton_salir.clicked.connect(self.salida) 
        self.boton_act.clicked.connect(self.actualizar_lista_usuario) 
        ...
        self.boton_act_2.clicked.connect(self.actualizar_lista_cliente) 
...
class acceso_punto_de_venta(QMainWindow):
    def __init__(self):
        super().__init__()
        uic.loadUi(r"Acceso_punto_de_venta.ui", self) 
        self.fn_init_UI()
    ...
    def revisar_acceso(self):
        ...
        if respuesta_bd != "Aprobado":
            self.alerta_acceso.setText("Usuario o contraseña incorrecta")
        else:
            self.punto_de_venta() 
